...
const getListingPrice = (property) => {
	...
	return `${numericPrice.toLocaleString()}/mo`;
};
...
export default function LatestForRent() {
	...
	useEffect(() => {
		...
		const fetchLatestForRent = async () => {
			setLoading(true);
			setError('');

			try {
				const response = await propertyApi.getLatestBySaleOption('rent');
				...
				setProperties([]);
				setError(fetchError?.message || 'Could not load latest properties rent.');
			} finally {
				if (mounted) {
					setLoading(false);
				}
	...
	return (
		<section className="pb-16 pt-4">
			<div className="mb-8 flex items-end justify-between gap-4">
				<div>
					<h2 className="text-3xl font-bold text-slate-800 sm:text-4xl">Latest rent</h2>
					<p className="mt-2 text-sm text-slate-500 sm:text-base">Fresh rental options in popular locations.</p>
				</div>
